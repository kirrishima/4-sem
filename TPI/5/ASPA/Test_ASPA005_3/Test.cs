    ...
    public class Test
    {
        ...
        async Task resultPRINT<T>(string method, string path, HttpResponseMessage rm, Func<T?, T?, int, string> result)
        {
            ...
            try
            {
                ...
                Console.WriteLine($" {r.ToString()}: {method} {path}, status = {status}, x = {x}, y = {y}, m = {answer?.message}");
            }
            catch (JsonException ex)
            {
                string r = result(default(T), default(T), status);
                Console.WriteLine($" {r.ToString()}: {method} {path}, status = {status}, x = {null}, y = {null}, m = {ex.Message}");
            }
        }
    }
}
