...
using (IRepository repository = Repository.Create("Celebrities"))
{
    ...
    .AddEndpointFilter(async (context, next) =>
    {
        ...
        if (repository.getCelebritiesBySurname(cel.Surname).Length > 0)
        {
    ...
    .AddEndpointFilter(async (context, next) =>
    {
        ...
        var pa = Path.Combine(Path.GetFileNameWithoutExtension(Repository.JSONFileName), cel.PhotoPath);

        if (!File.Exists(pa))
        {
            context.HttpContext.Response.Headers["X-Celebrity"] = $"NotFound = {Path.GetFileName(cel.PhotoPath)}";
        }
        return await next(context);
    });
    ...
    app.MapDelete("/Celebrities/{id:int}", (int id) =>
    {
        ...
        if (!deleted)
        {
            throw new DeleteByIdException($"не удалось удалить по id={id}");
        }
        return deleted;
    });
    ...
    app.Map("/Celebrities/Error", (HttpContext ctx) =>
    {
        ...
        if (ex != null)
        {
            ...
            if (ex is CelebrityIncorrectSurnameException)
